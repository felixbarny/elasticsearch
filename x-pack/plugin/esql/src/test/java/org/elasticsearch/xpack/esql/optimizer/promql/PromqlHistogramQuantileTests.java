...
public class PromqlHistogramQuantileTests extends AbstractPromqlPlanOptimizerTests {
    ...
    public void testHistogramQuantileLowersToClassicHistogramQuantileAggregate() {
        LogicalPlan translated = planHistogramPromql(
            "PROMQL index=prom_hist step=1m result=(histogram_quantile(0.9, sum by (job, le) (request_duration_seconds_bucket)))",
            false
        );
        List<ClassicHistogramQuantile> quantiles = collectQuantiles(translated);
    ...
    public void testSumHistogramQuantileUsesOuterAggregateOverClassicHistogramQuantile() {
        LogicalPlan translated = planHistogramPromql(
            "PROMQL index=prom_hist step=5m start=\"2024-05-10T00:10:00Z\" end=\"2024-05-10T00:10:00Z\" "
        ...
        List<ClassicHistogramQuantile> quantiles = translated.collect(TimeSeriesAggregate.class)
            .stream()
            .flatMap(timeSeriesAggregate -> timeSeriesAggregate.aggregates().stream())
            .flatMap(namedExpression -> namedExpression.collect(ClassicHistogramQuantile.class).stream())
            .toList();
        assertThat(quantiles, hasSize(1));
    }
    ...
    public void testHistogramQuantileWithoutLeUsesEvalAliasForUpperBound() {
        ...
        assertWarnings("histogram_quantile: input vector has no le label; no buckets to evaluate");
        ClassicHistogramQuantile quantile = translated.collect(TimeSeriesAggregate.class)
            .stream()
            .flatMap(timeSeriesAggregate -> timeSeriesAggregate.aggregates().stream())
            .flatMap(namedExpression -> namedExpression.collect(ClassicHistogramQuantile.class).stream())
            .findFirst()
    ...
    private static List<ClassicHistogramQuantile> collectQuantiles(LogicalPlan plan) {
