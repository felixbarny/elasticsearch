...
public class PromqlLogicalPlanOptimizerTests extends AbstractLogicalPlanOptimizerTests {
    ...
    public void testRangeSelector() {
        ...
        var plan = planPromql("""
            TS k8s
            | promql avg by (host.name) (avg_over_time(metrics.system.memory.utilization[1h]))
            """);
    ...
    public void testRate() {
        ...
        String testQuery = """
            TS k8s
            | promql
                avg by (host.name) (rate(metrics.system.cpu.time[1h]))
    ...
    public void testLabelSelectorPrefix() {
        ...
        String testQuery = """
            TS k8s
            | promql
                avg by (host_name)(avg_over_time(system_cpu_load_average_1m{host_name=~"host-.*"}[5m]))
            """;
