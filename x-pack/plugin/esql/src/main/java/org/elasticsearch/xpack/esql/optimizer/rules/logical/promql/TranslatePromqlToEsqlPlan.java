...
import org.elasticsearch.xpack.esql.expression.function.aggregate.ClassicHistogramQuantile;
...
public final class TranslatePromqlToEsqlPlan extends AnalyzerRules.ParameterizedAnalyzerRule<PromqlCommand, AnalyzerContext> {
    ...
    private TranslationResult translateHistogramQuantile(
        ...
        if (upperBound == null) {
            ...
            List<Attribute> leCandidates = new ArrayList<>(childResult.plan().output());
            childResult.plan().forEachDown(EsRelation.class, relation -> leCandidates.addAll(relation.output()));
            exportLabels = withoutClassicHistogramLe(childResult.labelSetSpec(), upperBound, leCandidates);
        }
        ...
        Expression aggregateExpression = new ClassicHistogramQuantile(
            histogramQuantile.source(),
            count,
    ...
    private static Attribute findClassicHistogramUpperBound(LogicalPlan plan) {
        ...
        plan.forEachDown(EsRelation.class, relation -> relationAttributes.addAll(relation.output()));
        return findAttributeByPromqlLabelName(relationAttributes, HistogramQuantile.LE_LABEL);
    }

    /**
     * Exclude every attribute that maps to the PromQL {@code le} label, including bare {@code le} and
     * Prometheus-style {@code labels.le} fields that share the same logical label name.
     */
    private static LabelSetSpec withoutClassicHistogramLe(LabelSetSpec spec, Attribute upperBound, List<Attribute> additionalCandidates) {
        List<Attribute> toExclude = new ArrayList<>();
        if (upperBound != null) {
            toExclude.add(upperBound);
        }
        for (Attribute attr : LabelSetSpec.unionByFieldName(spec.declared(), additionalCandidates)) {
            if (HistogramQuantile.LE_LABEL.equals(LabelSetSpec.promqlLabelKey(attr))) {
                toExclude.add(attr);
            }
        }
        for (Attribute attr : spec.excluded()) {
            if (HistogramQuantile.LE_LABEL.equals(LabelSetSpec.promqlLabelKey(attr))) {
                toExclude.add(attr);
            }
        }
        return LabelSetSpec.without(spec, toExclude);
    }

    private static Attribute findAttributeByPromqlLabelName(List<Attribute> attributes, String labelName) {
        return LabelSetSpec.findAttributeByFieldName(attributes, labelName);
    }
